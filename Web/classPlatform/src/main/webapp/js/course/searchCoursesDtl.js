...
//    	var url = getRootPath() + '/course/search?keyword='+keyword+'&minPrice='+minPrice+'&maxPrice='+maxPrice+'&pageNum='+(pageIndex+1);
    	var url = getRootPath() + '/course/search?pageNum='+(pageIndex+1);
    	if(keyword != ''){
	...
	$(".cor-sign").on("mouseover",function(){
		$(this).css("background","#1DA9F2");
	}).on("mouseout",function(){
		$(this).css("background","#42b8f6");
	});
...
function queryCourses(){
	...
	param={on:true, page:1, start:1,rows:10,'minPrice':minPrice, 'maxPrice':maxPrice, 'keyword':keyword}

var url;
	 url=getRootPath() + '/course/search';
		...
		success : function(data) {
			...
			var html='';
			 for(var i=0; i<courses.length; i++){
				 ...
				 html += '<div class="cor-title">'+cut_str(courses[i].courseName, 55)+'</div>';
