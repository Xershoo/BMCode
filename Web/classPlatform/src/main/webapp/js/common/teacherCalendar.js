...
function getCourseInfor(firstDateOfCalendarInfor,lastDateOfSelfCalendarInfor,line){
    var courseInforData = {
        ...
        $.ajax({
            url: getRootPath()+"/teacher/schedule/listCourseClass",//后端需要给的接口
            type: "post",
            dataType: "json",
    	                	...
    	                	if (key == weekStr[i]){
    	                		$.each(value, function(j, allData){
                                    ...
                                    if(allData.classState == 17){
                		    			...
                		    			str+='<div class="liInfor" style="display:block;text-align:left;padding-left:40px;display:none;cursor:pointer"><p class="name"   style="padding:15px 0 16px 0;">'+checkCourseNameLength(allData.courseName)+'</p><p class="time" style="padding-bottom:15px;"><span>第'+(j+1)+'节</span><span> '+allData.week+'</span><p style="color:#28c445;font-weight:bold"> '+allData.startTimePlan+'-'+allData.endTimePlan+'</p></p><p class="teacher" style="background:url('+getRootPath()+'/images/teacherBg.png) no-repeat 0 0px;padding-left:20px;font-size:12px;">'+allData.teacherName+'老师';
                		    		}
...
function getCourceById(){
	console.log("1111111");
    $.ajax({
        url: getRootPath()+"/teacher/schedule/listCourseName",//后端需要给的接口
        type: "post",
        dataType: "json",
        success: function(data) {
        	if(data.success){
